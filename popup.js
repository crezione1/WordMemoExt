...
async function toggleExtensionState() {
    if (!enableExtensionCheckbox) return;
    ...
    if (enableExtensionCheckbox.checked) {
        ...
        if (exclusionList) {
            exclusionList.childNodes.forEach((node) => {
                if (!(node.textContent && node.textContent.trim() === currentSiteHostname)) return;
...
function showNotification(message) {
    if (messageContainer && notificationContainer) {
...
function closeNotification() {
    if (notificationContainer && notificationContainer.classList.contains("notification-shown")) {
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    chrome.storage.local.get(['auth_token', 'token'], (result) => {
        if (result.auth_token || isTokenValid(result.token)) {
            ...
            showMainContent();
            getUserInfo(); // Load user info if authenticated
        } else {
    ...
    // Listen for auth state changes via storage changes
    chrome.storage.onChanged.addListener((changes, namespace) => {
        if (namespace === 'local' && (changes.auth_token || changes.user_info)) {
